...
void MarkFavorite(char contactNumber[], bool Status);

void ViewContactList(char queryFirstName[]);

int SaveChanges(struct Contact ReqContact, bool IsCreated);
...
void Dashboard() {
  ...
  case 5: {
    ...
    scanf("%s", Number);
    struct Contact info = FindByContact(Number);
    if (info.IsValid) {
      Details(info);
  ...
  case 6: {
    ...
    if (info.IsValid) {
      Details(info);
      printf("\t\t\tPress[Y] to True [N] to False mark favorite :: ");
      int command = getch();
      if (command == 89) {
        MarkFavorite(Number, true);
        printf("Contact updated Successfully");
      } else if (command == 78) {
        MarkFavorite(Number, false);
        printf("Contact updated Successfully");
      } else {
...
void UpdateContactDetail() {
  ...
  if (FoundContact.IsValid) {
    ...
    scanf("%d", & Favorite);
    fflush(stdin);
    Copied.IsFavorite = Favorite;
    EnsureTenDigitContactForUpdate(Copied.Number);
    SaveChanges(Copied, false);
...
void MarkFavorite(char contactNumber[], bool Status) {
  ...
  if (existingContact.IsValid) {
    ...
    editable.IsFavorite = Status;
    RemoveContact(existingContact.Number);
    SaveChanges(editable, false);
  } else {
    printf("\n\t\t\tContact not found\n");
  }
  Next();
